...
const SOCIAL_ICONS: Record<string, React.ReactNode> = {
  youtube: <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>,
  instagram: <svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/></svg>,
  facebook: <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>,
  ...
  whatsapp: <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347"/></svg>,
};

const DEFAULT_FOOTER: FooterSettings = {
  brandDescription: 'Master Shaman & Guardian of Our Planet. Guiding seekers home since 1991.',
  navigateLinks: [
  ...
  newsletter: {
    enabled: true,
    heading: 'Awaken Weekly',
    description: 'Receive potent insights directly from Abhay.',
    placeholder: 'Your email',
    successMessage: "You're subscribed. Welcome to the circle.",
    buttonText: 'Join',
  },
  copyrightText: 'Abhay Oyun. All Rights Reserved.',
  footerQuote: 'Walk in Power. Walk in Light.',
};

export default function Footer({ siteSettings }: { siteSettings?: { footer?: FooterSettings; navigation?: { logoText?: string }; topBar?: { socialLinks?: SocialLink[] } } }) {
  const year = new Date().getFullYear();
  const [newsEmail, setNewsEmail] = useState('');
  const [newsStatus, setNewsStatus] = useState<'idle' | 'sending' | 'success' | 'error'>('idle');
  const [newsError, setNewsError] = useState('');

  const footerData = siteSettings?.footer || DEFAULT_FOOTER;
  const brandDescription = footerData.brandDescription || DEFAULT_FOOTER.brandDescription!;
  const navigateLinks = footerData.navigateLinks || DEFAULT_FOOTER.navigateLinks!;
  const resourceLinks = footerData.resourceLinks || DEFAULT_FOOTER.resourceLinks!;
  const newsletter = footerData.newsletter || DEFAULT_FOOTER.newsletter!;
  const copyrightText = footerData.copyrightText || DEFAULT_FOOTER.copyrightText!;
  const footerQuote = footerData.footerQuote || DEFAULT_FOOTER.footerQuote!;
  const logoText = siteSettings?.navigation?.logoText || 'Abhay Oyun';
  const socialLinks = siteSettings?.topBar?.socialLinks || [];
              ...
              {socialLinks.map(s => (
                ...
                >
                  {SOCIAL_ICONS[s.platform] || (
                    <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="none"/><path d="M12 8v8M8 12h8" stroke="currentColor" strokeWidth="2"/></svg>
                  )}
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            {newsStatus === 'success' ? (
              ...
              <form onSubmit={handleSubscribe} style={{ display: 'flex' }}>
                ...
                >
                  {newsStatus === 'sending' ? '...' : newsStatus === 'error' ? 'Retry' : newsletter.buttonText}
                </button>
              </form>
            )}
            {newsStatus === 'error' && (
