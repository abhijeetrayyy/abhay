...
export default function WebinarCTASection({ sanity }: { sanity?: Record<string, any> }) {
  ...
  return (
    <section style={{ background: "#FBF9F5", position: "relative", marginTop: "-1px", zIndex: 20 }}>
      <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 80% 60% at 70% 50%, rgba(201,160,74,0.06) 0%, transparent 70%)", pointerEvents: "none" }} />
        ...
        <div className="webinar-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 100, alignItems: "center" }}>
          <motion.div initial={{ opacity: 0, y: 36 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 1.0, ease: [0.16, 1, 0.3, 1] }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 32 }}>
              ...
              <span style={{ fontFamily: "'Cinzel', serif", fontSize: "0.65rem", fontWeight: 600, letterSpacing: "0.28em", textTransform: "uppercase" as const, color: "#A07D2E" }}>{eyebrow}</span>
            </div>
            <h2 style={{ margin: "0 0 28px 0", fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 400, fontSize: "clamp(2rem, 7vw, 3.8rem)", letterSpacing: "-0.02em", color: "#1F1B16", lineHeight: 0.95 }}>
              {heading}<br />
              <span style={{ fontStyle: "italic", background: "linear-gradient(135deg, #E8CD7A, #C9A04A, #A07D2E)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>{subheading}</span>
            </h2>
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column" as const, gap: 18 }}>
              {benefits.map((b: string, i: number) => (
                <motion.li key={i} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.3 + i * 0.1 }} style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 22, height: 22, borderRadius: "50%", border: "1px solid rgba(201,160,74,0.3)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <svg width="9" height="9" viewBox="0 0 24 24" fill="#C9A04A"><path d="M12 2L14.5 9.5L22 12L14.5 14.5L12 22L9.5 14.5L2 12L9.5 9.5L12 2Z" /></svg>
                  </div>
            ...
            <div style={{ background: "#FDFCFA", border: "1px solid rgba(31,27,22,0.06)", borderRadius: 2, padding: "clamp(28px, 4vw, 48px)", position: "relative", overflow: "hidden", boxShadow: "0 30px 60px rgba(31,27,22,0.05)" }}>
              ...
              <div style={{ position: "relative" }}>
                <div style={{ marginBottom: 26 }}>
                  <div style={{ fontFamily: "'Cinzel', serif", fontSize: "0.62rem", fontWeight: 600, letterSpacing: "0.25em", textTransform: "uppercase" as const, color: "#A07D2E", marginBottom: 8 }}>{cardLabel}</div>
