...
export default function TransformationSection({ sanity }: { sanity?: Record<string, any> }) {
  ...
  return (
    <section ref={ref} id="transformation" className="transformation-section" style={{ position: "relative", background: "#FBF9F5", marginTop: "-1px", zIndex: 20 }}>
          ...
          <div className="sticky-quote" style={{ position: "sticky", top: 140, alignSelf: "start" }}>
            <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1.1, ease: [0.16, 1, 0.3, 1] }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 40 }}>
                ...
                <span style={{ fontFamily: "'Cinzel', serif", fontSize: "0.62rem", fontWeight: 600, letterSpacing: "0.28em", textTransform: "uppercase" as const, color: "#A07D2E" }}>{eyebrow}</span>
              </div>
              <blockquote style={{ margin: "0 0 48px 0" }}>
                <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontStyle: "italic", fontSize: "clamp(1.8rem, 5.5vw, 3.2rem)", lineHeight: 1.2, letterSpacing: "-0.02em", margin: "0 0 24px 0", fontWeight: 400, color: "#1F1B16" }}>
                  &ldquo;{quote}&rdquo;
                </p>
          ...
          <motion.div initial={{ opacity: 0, x: 32 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1.1, delay: 0.15 }}
            ...
            {milestones.map((m: any, i: number) => (
              <motion.div key={m.year || i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                ...
                <div style={{ position: "absolute", left: -38, top: 7, width: 14, height: 14, borderRadius: "50%", border: "1.5px solid rgba(201,160,74,0.3)", background: "#FBF9F5", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#C9A04A" }} />
                </div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 14, marginBottom: 12 }}>
                  <span style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "1.4rem", color: "#C9A04A", fontWeight: 400 }}>{m.year}</span>
                ...
                <motion.div initial={{ boxShadow: "0 8px 24px rgba(31,27,22,0.06)" }} whileHover={{ boxShadow: "0 18px 44px rgba(201,160,74,0.12)", borderColor: "rgba(201,160,74,0.25)" }}
