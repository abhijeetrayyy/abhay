...
function SpiritWolf({ accentColor = '#C9A04A' }: { accentColor?: string }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 3, delay: 1 }}
      ...
      <svg width="min(35vw, 320px)" height="min(25vw, 220px)" viewBox="0 0 500 350" fill="none">
        {["M80,280 120,240 160,260 200,220 240,240 280,200 320,220 360,180 400,200","M200,220 220,180 250,160 280,180 280,200","M120,240 100,200 130,180 160,200"].map((d, i) => (
          <motion.path key={i} d={d} stroke={accentColor} strokeWidth="0.4" fill="none"
        ...
        {[[250,160],[280,180],[200,220],[240,240],[320,220],[160,260]].map(([cx, cy], i) => (
          <motion.circle key={i} cx={cx} cy={cy} r={2} fill={accentColor} initial={{ opacity: 0 }} animate={{ opacity: 0.7 }} transition={{ delay: 3 + i * 0.3 }} />
        ))}
        <motion.circle cx={250} cy={160} r={3} fill={accentColor} animate={{ opacity: [0.3, 1, 0.3], r: [2, 5, 2] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut', delay: 4 }} />
      </svg>
    </motion.div>
  );
}
...
function LightSweep({ accentColor = '#C9A04A' }: { accentColor?: string }) {
  return (
    <motion.div animate={{ x: ['-100%', '200%'], opacity: [0, 0.06, 0] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut', delay: 2 }}
      style={{ position: 'absolute', top: 0, bottom: 0, width: '30%', zIndex: 7, pointerEvents: 'none', background: `linear-gradient(90deg, transparent, ${accentColor}0A, transparent)`, transform: 'skewX(-20deg)' }} />
  );
}

function Stars({ accentColor = '#C9A04A' }: { accentColor?: string }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 5, pointerEvents: 'none', overflow: 'hidden' }}>
      {starData.map((s, i) => (
        <motion.div key={i} style={{ position: 'absolute', left: s.left, top: s.top, width: s.size, height: s.size, borderRadius: '50%', background: s.type === 0 ? `${accentColor}80` : s.type === 1 ? 'rgba(255,255,255,0.3)' : 'rgba(180,210,255,0.3)' }}
...
function SacredGeometry({ accentColor = '#C9A04A' }: { accentColor?: string }) {
  const shouldReduce = useReducedMotion();
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 6, pointerEvents: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <motion.div animate={shouldReduce ? {} : { rotate: 360 }} transition={{ duration: 100, repeat: Infinity, ease: 'linear' }}
        style={{ position: 'absolute', width: 'min(85vw, 700px)', height: 'min(85vw, 700px)', border: `1px solid ${accentColor}08`, borderRadius: '50%' }} />
      <motion.div animate={shouldReduce ? {} : { rotate: -360 }} transition={{ duration: 70, repeat: Infinity, ease: 'linear' }}
        style={{ position: 'absolute', width: 'min(65vw, 540px)', height: 'min(65vw, 540px)', border: `1px solid ${accentColor}0D`, borderRadius: '50%' }} />
      <motion.div animate={shouldReduce ? {} : { scale: [1, 1.1, 1], opacity: [0.015, 0.08, 0.015] }} transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut' }}
        style={{ position: 'absolute', width: 'min(50vw, 420px)', height: 'min(50vw, 420px)', border: `2px solid ${accentColor}08`, borderRadius: '50%' }} />
    </div>
  );
}

function FloatingRunes({ accentColor = '#C9A04A' }: { accentColor?: string }) {
  const [m, setM] = useState(false);
  const shouldReduce = useReducedMotion();
  useEffect(() => { setM(true); }, []);
  if (!m) return null;
    ...
    <>
      {runes.map((rune, i) => (
        <motion.div key={i} initial={{ y: '110vh', opacity: 0, rotate: 0 }}
          ...
          style={{ position: 'absolute', left: `${15 + i * 14}%`, zIndex: 6, pointerEvents: 'none', color: accentColor, fontSize: `${12 + i * 2}px`, fontFamily: 'serif', opacity: 0.04, filter: 'blur(1px)', textShadow: `0 0 20px ${accentColor}1A` }}>
          {rune}
        </motion.div>
      ))}
    </>
...
function ScrollIn({ accentColor = '#C9A04A' }: { accentColor?: string }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 4 }}
      ...
      <motion.div animate={{ y: [0, 4, 0] }} transition={{ duration: 2, repeat: Infinity }} style={{ width: 16, height: 24, border: `1px solid ${accentColor}26`, borderRadius: 8, display: 'flex', justifyContent: 'center', paddingTop: 3 }}>
        <motion.div animate={{ opacity: [1, 0.2, 1], y: [0, 4, 0] }} transition={{ duration: 2, repeat: Infinity }} style={{ width: 2, height: 5, background: accentColor, borderRadius: 1 }} />
      </motion.div>
    </motion.div>
  );
}
...
function GalleryMarquee({ accentColor = '#C9A04A' }: { accentColor?: string }) {
  ...
  return (
    <div className="hero-marquee" style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 8, overflow: 'hidden', height: 160 }}>
      ...
      <motion.div className="hero-marquee-inner" animate={{ x: ['0%', '-33.33%'] }} transition={{ duration: 80, repeat: Infinity, ease: 'linear' }}
        ...
        {double.map((src, i) => (
          <motion.div key={i} whileHover={{ scale: 1.08, opacity: 1, y: -4 }} style={{ flexShrink: 0, width: 130, height: 95, borderRadius: 6, overflow: 'hidden', opacity: 0.45, border: `1px solid ${accentColor}1F`, position: 'relative', boxShadow: '0 4px 20px rgba(0,0,0,0.2)', transition: 'opacity 0.4s ease' }}>
...
export default function HeroSection({ sanity }: { sanity?: Record<string, any> & SectionStylingData }) {
  ...
  return (
    <section ref={ref} className="hero-section" style={{ position: 'relative', height: '100svh', minHeight: 'min(800px, 100dvh)', background: 'linear-gradient(170deg, #080C1A 0%, #0F1A30 40%, #0A1020 100%)', display: 'flex', alignItems: 'center', overflow: 'hidden', ...sectionStyle }}>
      ...
      {ready && <LightSweep accentColor={accentColor} />}
      {ready && <FloatingRunes accentColor={accentColor} />}
      {ready && <SpiritWolf accentColor={accentColor} />}
      {ready && <GalleryMarquee accentColor={accentColor} />}
      ...
      <motion.div style={{ position: 'relative', zIndex: 20, textAlign: 'left', y: ready ? textY : '0%', width: '100%', maxWidth: 1200, padding: '0 clamp(24px, 6vw, 80px)', marginTop: 40 }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={ready ? { opacity: 1, y: 0 } : {}} transition={{ duration: 1, delay: 0.3 }}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 12, padding: '8px 20px', borderRadius: 2, border: `1px solid ${accentColor}1F`, background: 'rgba(8,12,26,0.4)', backdropFilter: 'blur(16px)', marginBottom: 28 }}>
          <motion.span animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 2, repeat: Infinity }} style={{ width: 4, height: 4, borderRadius: '50%', background: accentColor, boxShadow: `0 0 8px ${accentColor}66` }} />
          <span style={{ fontFamily: "'Cinzel', serif", fontSize: '0.52rem', fontWeight: 500, letterSpacing: '0.3em', textTransform: 'uppercase', color: `${accentColor}B3` }}>{tagline}</span>
          <motion.span animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 2, repeat: Infinity, delay: 0.5 }} style={{ width: 4, height: 4, borderRadius: '50%', background: accentColor, boxShadow: `0 0 8px ${accentColor}66` }} />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 50 }} animate={ready ? { opacity: 1, y: 0 } : {}} transition={{ duration: 1.4, delay: 0.5, ease: [0.16, 1, 0.3, 1] }}>
          <h1 style={{ margin: 0, fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 200, fontSize: 'clamp(3.5rem, 10vw, 7.5rem)', letterSpacing: '-0.03em', lineHeight: 1.02, color: '#FDFCFA', textShadow: '0 2px 40px rgba(8,12,26,0.5)' }}>
            {heading}<br />
            <span style={{ fontStyle: 'italic', backgroundImage: `linear-gradient(135deg, #F9D58B 0%, ${accentColor} 35%, #A07D2E 65%, #E8CD7A 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{subheading}</span>
          </h1>
        </motion.div>
