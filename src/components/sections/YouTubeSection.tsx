...
function VideoCard({ video, index }: { video: { id: string; title: string; subtitle: string }; index: number }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 48 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }}
      ...
      style={{ borderRadius: 2, overflow: "hidden", background: "#FDFCFA", border: "1px solid rgba(31,27,22,0.06)", position: "relative", cursor: "pointer",
        boxShadow: hovered ? "0 20px 50px rgba(0,0,0,0.15), 0 0 0 1px rgba(201,160,74,0.2)" : "0 6px 24px rgba(0,0,0,0.06)", transition: "box-shadow 0.4s ease" }}>
      {playing ? (
        ...
        <div style={{ position: "relative", paddingBottom: "56.25%", cursor: "pointer" }} onClick={() => setPlaying(true)}>
          ...
          <motion.div animate={{ scale: hovered ? 1.1 : 1 }} transition={{ duration: 0.3 }}
            style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: 56, height: 56, borderRadius: "50%",
              background: hovered ? "#C9A04A" : "rgba(253,252,250,0.7)", backdropFilter: "blur(8px)", border: "1.5px solid rgba(253,252,250,0.25)",
      ...
      <div style={{ padding: "16px 18px" }}>
        <div style={{ fontFamily: "'Cinzel', serif", fontSize: "0.58rem", fontWeight: 600, letterSpacing: "0.22em", textTransform: "uppercase" as const, color: "#A07D2E", marginBottom: 5 }}>{video.subtitle}</div>
...
export default function YouTubeSection({ sanity }: { sanity?: Record<string, any> }) {
      ...
      <div className="yt-wrapper" style={{ maxWidth: 1440, margin: "0 auto", padding: "140px 7vw" }}>
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.9 }}
          ...
          <div>
            ...
            <h2 style={{ margin: 0, fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 400, fontSize: "clamp(2rem, 7vw, 4.2rem)", letterSpacing: "-0.03em", color: "#1F1B16", lineHeight: 0.95 }}>
