...
export default function FinalCTASection({ sanity }: { sanity?: Record<string, any> }) {
    ...
    <section style={{ position: "relative", background: '#F5F1EA', overflow: 'hidden', borderTop: '1px solid rgba(31,27,22,0.04)' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 'clamp(80px, 12vw, 140px) 24px', position: 'relative', textAlign: 'center' }}>
        ...
        <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.1 }}
          style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 400, fontSize: 'clamp(2.2rem, 5vw, 3.8rem)', color: '#1F1B16', lineHeight: 1.15, marginBottom: 24, letterSpacing: '-0.01em' }}>
          {heading}
        </motion.h2>
