...
export default function ShamanIntroSection({ sanity }: { sanity?: Record<string, any> & SectionStylingData }) {
        ...
        <div className="shaman-intro-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "clamp(40px, 8vw, 100px)", alignItems: "center" }}>
          <motion.div style={{ y: shouldReduce ? 0 : textY }} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: "clamp(20px, 3vh, 36px)" }}>
              ...
              <span style={{ fontFamily: "'Cinzel', serif", fontSize: "clamp(0.62rem, 0.85vw, 0.72rem)", fontWeight: 600, letterSpacing: "0.28em", textTransform: "uppercase", color: accentColor }}>{eyebrow}</span>
            </div>
            <h2 style={{ margin: "0 0 clamp(24px, 4vh, 40px) 0", padding: 0, fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 400, lineHeight: 1.06, letterSpacing: "-0.02em", color: "inherit", fontSize: "clamp(2.4rem, 7vw, 4.8rem)" }}>
            ...
            <div className="stats-row" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", borderTop: `1px solid ${accentColor}26`, marginBottom: "clamp(36px, 5vh, 52px)" }}>
              {stats.map((s: any, i: number) => (
                <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  transition={{ duration: 0.7, delay: i * 0.15, ease: [0.16, 1, 0.3, 1] }}
                  className="stats-col" style={{ padding: "clamp(20px, 3vh, 30px)", borderRight: i < 2 ? `1px solid ${accentColor}1A` : "none" }}>
                  <div style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(1.8rem, 3vw, 2.6rem)", fontWeight: 400, background: `linear-gradient(135deg, ${accentColor}, #A07D2E)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>{s.number}</div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14 }}>
              <Link href={primaryUrl} target="_blank"
                ...
                <svg width="15" height="15" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7.25" stroke={accentColor} strokeWidth="1.5" /><path d="M6 5l5 3-5 3V5z" fill={accentColor} /></svg>
                {primaryLabel}
              </Link>
          ...
          <div style={{ position: "relative" }}>
            <div style={{ position: "absolute", top: -28, right: -28, width: "65%", height: "65%", background: accentColor, opacity: 0.04, borderRadius: 2, zIndex: 0 }} />
            <div style={{ position: "absolute", bottom: -28, left: -28, width: "45%", height: "45%", background: `radial-gradient(circle, ${accentColor}1F 0%, transparent 70%)`, borderRadius: "50%", zIndex: 0 }} />
              ...
              <div style={{ borderRadius: 2, overflow: "hidden", boxShadow: "0 40px 80px rgba(0,0,0,0.1)", aspectRatio: "3/4", position: "relative", background: "#F5F1EA" }}>
                ...
                <div style={{ position: "absolute", bottom: 28, left: 28, right: 28 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                    <div style={{ width: 36, height: 1, background: accentColor }} />
                    <span style={{ fontFamily: "'Cinzel', serif", fontSize: "0.62rem", letterSpacing: "0.2em", textTransform: "uppercase", color: accentColor }}>{imageCaption}</span>
                  </div>
            ...
            <motion.div style={{ position: "absolute", top: -20, left: -20, zIndex: 5 }} initial={{ opacity: 0, scale: 0 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.4 }}>
              <motion.div animate={shouldReduce ? {} : { y: [0, -8, 0], rotate: [0, 5, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }} style={{ opacity: 0.5 }}>
                <svg width="52" height="52" viewBox="0 0 100 100" fill="none" stroke={accentColor} strokeWidth="1.2"><circle cx="50" cy="50" r="45" /><polygon points="50,85 18,35 88,60 12,60 82,35" /></svg>
              </motion.div>
            </motion.div>
          </div>
        </div>
