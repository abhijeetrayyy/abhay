      ...
      <div className={containerClass}>
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} style={{ textAlign: 'center', marginBottom: 'clamp(40px, 6vw, 64px)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 18 }}>
            <div style={{ width: 44, height: 1, background: `linear-gradient(to right, ${accentColor}, transparent)` }} />
            ...
            <div style={{ width: 44, height: 1, background: `linear-gradient(to left, ${accentColor}, transparent)` }} />
          </div>
          <h2 style={{ margin: 0, fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 400, fontSize: 'clamp(2rem, 4.5vw, 3.2rem)', color: 'inherit', lineHeight: 1.1, letterSpacing: '-0.01em' }}>{heading}</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          <motion.div key={featured.title} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>
            <div style={{ background: '#FDFCFA', borderRadius: 2, overflow: 'hidden', border: `1px solid ${accentColor}4D`, boxShadow: `0 8px 40px ${accentColor}14, 0 0 0 1px ${accentColor}1A`, position: 'relative' }}>
              {featured.tag && (
                <div style={{ position: 'absolute', top: 14, right: 14, zIndex: 5, display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 2, background: `${accentColor}1F`, backdropFilter: 'blur(8px)', border: `1px solid ${accentColor}33` }}>
                  <span style={{ color: accentColor, fontSize: '0.55rem' }}>★</span>
                  <span style={{ fontFamily: "'Cinzel', serif", fontSize: '0.5rem', fontWeight: 600, color: accentColor, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Recommended</span>
                </div>
              )}
              <div className="programs-featured-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 300 }}>
                <div className="programs-featured-img" style={{ position: 'relative', height: '100%', minHeight: 300, overflow: 'hidden' }}>
                  ...
                  <span style={{ position: 'absolute', top: 14, left: 14, background: `linear-gradient(135deg, ${accentColor}, #A07D2E)`, color: '#FDFCFA', padding: '5px 12px', borderRadius: 2, fontSize: '0.6rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', fontFamily: "'Cinzel', serif" }}>{featured.tag || 'Popular'}</span>
                </div>
                <div style={{ padding: 32, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <span style={{ color: accentColor, fontSize: '0.62rem', fontWeight: 600, letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: "'Cinzel', serif" }}>{featured.subtitle}</span>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                    <span style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '1.5rem', color: accentColor, fontWeight: 500 }}>{featured.price}</span>
                    <Link href={featured.link || '/events'} style={{ background: `linear-gradient(135deg, ${accentColor}, #A07D2E)`, color: '#FDFCFA', padding: '12px 28px', borderRadius: 2, textDecoration: 'none', fontSize: '0.68rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', fontFamily: "'Cinzel', serif" }}>Learn More</Link>
                  </div>
                </div>
              </div>
            </div>
                ...
                <motion.div key={prog.title} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (i + 1) * 0.12, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>
                  <div style={{ background: '#FDFCFA', borderRadius: 2, overflow: 'hidden', border: '1px solid rgba(31,27,22,0.06)', boxShadow: '0 4px 30px rgba(0,0,0,0.04)' }}>
                    ...
                    <div style={{ padding: 24 }}>
                      <span style={{ color: accentColor, fontSize: '0.6rem', fontWeight: 600, letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: "'Cinzel', serif" }}>{prog.subtitle}</span>
